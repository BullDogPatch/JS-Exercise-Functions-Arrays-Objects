...
function getCarInfoByIndex(inventory, index) {
  /* code here */
  let carIndex = inventory.find((item, index) => {
    return index === index;
  })
...
function getModelYears(inventory) {
  /* code here */
  //   var carYears = [];
  //   for (carYears i = 0; i < inventory.length; ++i)
...
function getOlderCars(inventory, num) {
  ...
  for (let i in inventory) {
    inventory[i].car_year <= num ? carYear.push(inventory[i]) : undefined;
  }
  return carYear;
}
//   let carYear = [];
//   for (var i = 0; i <= num; i++) {
//     carYear.push(inventory[i].car_year) : undefined;
//   }
//   return carYear
// }
